...
#include "triton-shared/Dialect/Triton/Utils/Utils.h"
...
struct AdvanceConverter : public OpConversionPattern<triton::AdvanceOp> {
  ...
  matchAndRewrite(triton::AdvanceOp op, OpAdaptor adaptor,
    ...
    llvm::SmallDenseMap<Value, triton::PtrState> knownPtrs;
    triton::PtrState pointerState;
    triton::PtrAnalysis::rewriteAdvanceOp(op, rewriter, knownPtrs);
    return success();
  }
};
...
struct LegacyAddPtrConverter : public OpConversionPattern<triton::AddPtrOp> {
  ...
  matchAndRewrite(triton::AddPtrOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    llvm::SmallDenseMap<Value, triton::PtrState> knownPtrs;
    triton::PtrAnalysis::rewriteAddptrOp(op, rewriter, knownPtrs);
    return success();
  }
};
...
public:
  ...
  matchAndRewrite(triton::LoadOp op, OpAdaptor adaptor,
    ...
    if (!isa<ShapedType>(op.getResult().getType())) {
      auto sMemRef = triton::PtrAnalysis::getScalarMemRef(
          op.getPtr(), adaptor.getPtr(), loc, rewriter);
    ...
    if (!mask) {
      ...
      if (auto unrealizedCast =
        ...
        if (auto wrapType = unrealizedCast->getAttrOfType<StringAttr>(
                triton::ModuloState::WraparoundAttr)) {

          auto memrefs = unrealizedCast.getOperands();
          auto block1 = memrefs[0];
          auto block2 = memrefs[1];

          if (wrapType.getValue() ==
              triton::ModuloState::WraparoundSideBySide) {
            createSideBySideCopies(block1, block2, alloc, loc, rewriter);
          } else if (wrapType.getValue() ==
                     triton::ModuloState::WraparoundStacked) {
    ...
    if (auto unrealizedCast = ptr.getDefiningOp<UnrealizedConversionCastOp>()) {
      if (auto wrapType = unrealizedCast->getAttrOfType<StringAttr>(
        ...
        if (wrapType.getValue() == triton::ModuloState::WraparoundSideBySide) {
...
struct StoreConverter : public OpConversionPattern<triton::StoreOp> {
  ...
  matchAndRewrite(triton::StoreOp op, OpAdaptor adaptor,
    ...
    if (!isa<ShapedType>(val.getType())) {
      auto sMemRef =
          triton::PtrAnalysis::getScalarMemRef(op.getPtr(), ptr, loc, rewriter);
      auto zeroMap = AffineMap::getConstantMap(0, rewriter.getContext());
...
struct LoopConverter : public OpConversionPattern<scf::ForOp> {
  ...
  matchAndRewrite(scf::ForOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    llvm::SmallDenseMap<Value, triton::PtrState> knownPtrs;
    triton::PtrAnalysis::IndexMapSet
        levelToBlockArgIndex; // level -> set of block arg index to be replaced

    triton::PtrAnalysis::rewriteForOp(op, rewriter, levelToBlockArgIndex, 0,
                                      knownPtrs);
    return success();
  }
};
...
public:
  ...
  matchAndRewrite(triton::BroadcastOp op, OpAdaptor adaptor,
    ...
    indexingMaps.push_back(triton::getBroadcastAffineMap(
    ...
    auto linalgOp = rewriter.create<linalg::GenericOp>(
        ...
        triton::getNParallelLoopsAttrs(resultRank),
    ...
    linalgOp->setAttr("broadcastDims",
                      rewriter.getDenseI64ArrayAttr(
                          triton::getBroadcastDims(sourceType, resultType)));
...
struct MakeRangeConverter : public OpConversionPattern<triton::MakeRangeOp> {
  ...
  matchAndRewrite(triton::MakeRangeOp op, OpAdaptor adaptor,
    ...
    auto linalgOp = rewriter.create<linalg::GenericOp>(
        ...
        ValueRange{init}, indexingMaps, triton::getNParallelLoopsAttrs(1),
        [&](OpBuilder &nestedBuilder, Location nestedLoc,
            ValueRange blockArgs) {
...
struct AssertConverter : public OpConversionPattern<triton::AssertOp> {
  ...
  matchAndRewrite(triton::AssertOp op, OpAdaptor adaptor,
    ...
    if (isa<mlir::IntegerType>(condVal.getType())) {
      // handle scalar case
      rewriter.create<cf::AssertOp>(op.getLoc(), condVal, assertMessage.str());
      ...
      rewriter.create<linalg::GenericOp>(
          ...
          [&](OpBuilder &b, Location loc, ValueRange args) {
            ...
            b.create<cf::AssertOp>(loc, element, assertMessage.str());
...
struct CallConverter : public OpConversionPattern<triton::CallOp> {
  ...
  matchAndRewrite(triton::CallOp op, OpAdaptor adaptor,
    ...
    if (triton::FuncOp parentFunc = op->getParentOfType<triton::FuncOp>()) {
      ...
      if (ModuleOp module = op->getParentOfType<ModuleOp>()) {
        if (triton::FuncOp calleeFunc =
                module.lookupSymbol<triton::FuncOp>(calleeName)) {
...
class TritonArithToLinalgPass
    : public mlir::triton::impl::TritonArithToLinalgBase<
          TritonArithToLinalgPass> {
  using TritonArithToLinalgBase<
      TritonArithToLinalgPass>::TritonArithToLinalgBase;

  static constexpr uint32_t LAUNCH_GRID_RANK =
      triton::getMaxEnumValForProgramIDDim() + 1;
  static constexpr uint32_t TRITON_PROGRAM_INFO_ARG_COUNT =
      LAUNCH_GRID_RANK * 2;
  ...
  static void addProgramInfo(triton::FuncOp func) {
    ...
    for (uint32_t i = 0; i < TRITON_PROGRAM_INFO_ARG_COUNT; i++) {
...
public:
  void runOnOperation() override {
...

    {
      ...
      triton::populateTritonArithToLinalgCanonicalizationPatterns(patterns);
      if (failed(applyPatternsGreedily(moduleOp, std::move(patterns)))) {
        signalPassFailure();
      }
    }
    ...
    if (tensorPtrToLinalg) {
      ...
      triton::populateTritonTensorPtrConversionPatterns(patterns);
    }
