...
struct EmptyTensorConverter : public OpConversionPattern<tensor::EmptyOp> {
  ...
  matchAndRewrite(tensor::EmptyOp op, OpAdaptor adaptor,
    ...
    rewriter.replaceOpWithNewOp<tensor::EmptyOp>(
        ...
        ptr::PtrType::get(ptr::GenericSpaceAttr::get(rewriter.getContext())));
    return success();
  }
};
...
struct LoadConverter : public OpConversionPattern<triton::LoadOp> {
  ...
  matchAndRewrite(triton::LoadOp op, OpAdaptor adaptor,
    ...
    auto memref = rewriter.create<ptr::FromPtrOp>(
...
struct PtrToIntConverter : public OpConversionPattern<triton::PtrToIntOp> {
  ...
  matchAndRewrite(triton::PtrToIntOp op, OpAdaptor adaptor,
    ...
    auto reinterpretCastOp = UnrealizedConversionCastOp::create(
        rewriter, op->getLoc(), resultType, adaptor.getSrc());
    rewriter.replaceOp(op, reinterpretCastOp);
    return success();
  }
};
...
class TritonToPtrPass
    : public mlir::triton::impl::TritonToPtrBase<TritonToPtrPass> {
  using TritonToPtrBase<TritonToPtrPass>::TritonToPtrBase;
