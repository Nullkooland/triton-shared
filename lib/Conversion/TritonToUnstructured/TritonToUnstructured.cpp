...
static Type getPtrOffsetType(Type type, uint32_t bitWidth) {
...
static uint32_t getBitWidth(Type type) {
  if (auto tensorType = dyn_cast<RankedTensorType>(type)) {
...
class TritonToUnstructuredPass
  ...
  using TritonToUnstructuredBase<
      TritonToUnstructuredPass>::TritonToUnstructuredBase;
...
public:
  ...
  LogicalResult processUnstructuredPtrs(uint32_t defaultBitWidth = 32) {
        ...
        auto res =
            llvm::TypeSwitch<Operation *, LogicalResult>(user)
                ...
                .Default([&](Operation *op) {
                  op->emitWarning("unexpected op in ptr sequence");
                  return failure();
                });
